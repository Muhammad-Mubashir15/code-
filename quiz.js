...
let timerId = setInterval(() => {
  document.getElementById('timer').innerText = `Your remaining time is 00:${time}`;
  time--;
  if (time <= 0) {
...
function nextQuestion() {
    ...
    //    console.log((score / questions.length) * 100+"%")
       var grade=(score / questions.length) * 100+"%"
       if(grade >='60%'){
        Swal.fire({
            title: grade,
            text: "You are Passed Congrats...",
            icon: "success"
          });
    }
    else{
        Swal.fire({
            ...
            title: grade,
            text: "You are fail.",
            footer: '<a href="#">You need more learning..</a>'
          });
    }
    }
